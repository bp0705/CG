...
*                          loadTextures                          *
*****************************************************************/
//Purpose: Loads all textures from files
void loadTextures(void)
{
	hexagonTex = SOIL_load_OGL_texture("hexagon.jpg", 4, 0, SOIL_FLAG_MIPMAPS);
	if (!hexagonTex) {
	printf("***NO BITMAP RETRIEVED***\n");  //Check to see if successfully loaded
	exit(1);

	thrusterFrontTex = SOIL_load_OGL_texture("thrusterFront.jpg", 4, 0, 0);
	if (!thrusterFrontTex) {
	printf("***NO BITMAP RETRIEVED***\n");  //Check to see if successfully loaded
	exit(1);
  }
